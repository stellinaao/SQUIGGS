    ...
    def __init__(
        ...
        if self.mode == "grand":
            ...
            self.trace_std = self.ax.fill_between(
                self.times, mean - std, mean + std, alpha=0.5, color="#5C5EA1"
            )
            ...
            # plot means
            (self.trace_mean_a,) = self.ax.plot(
                self.times, mean_a, color="#29723E", label=label_a
            )
            (self.trace_mean_b,) = self.ax.plot(
                self.times, mean_b, color="#672982", label=label_b
            )

            # plot stds
            self.trace_std_a = self.ax.fill_between(
                self.times, mean_a - std_a, mean_a + std_a, alpha=0.5, color="#6FCD77"
            )

            self.trace_std_b = self.ax.fill_between(
                self.times, mean_b - std_b, mean_b + std_b, alpha=0.5, color="#9F5DBCFF"
            )
            ...
            ymin = np.min(
                (
                    np.min(self.all_means_a - self.all_stds_a),
                    np.min(self.all_means_b - self.all_stds_b),
                )
            )
            ymax = np.max(
                (
                    np.max(self.all_means_a + self.all_stds_a),
                    np.max(self.all_means_b + self.all_stds_b),
                )
            )
